...
!      iy=(rlat-known_lat)/dy
!      ix=(rlon-known_lon)/dx

      if (mod(ix,tile_x).ne.0) then 
        isx=(ix/tile_x)*tile_x+1
      else
        isx=(ix/tile_x-1)*tile_x+1
      endif
      if (mod(iy,tile_y).ne.0) then 
        isy=(iy/tile_y)*tile_y+1
      else
        isy=(iy/tile_y-1)*tile_y+1
      endif
      write (sx,'(I5.5)') isx
      write (sy,'(I5.5)') isy
      ...
      sfile=sx//'-'//ex//'.'//sy//'-'//ey
      !print *,'isx,isy,ix,iy',isx,isy,ix,iy
     ...
     +           isigned, endian, scalefactor, wordsize, ierr)
      ...
      if (ix-isx+1.gt.0 .and. iy-isy+1.gt.0) then 
